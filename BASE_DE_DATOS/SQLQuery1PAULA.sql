...
--De cada cliente la cantidad de pedidos que hicieron

SELECT CONCAT(c.name, '', c.last_name) AS nombre_completo, COUNT(o.number_order) AS cantidad_pedidos FROM clients c
JOIN orders o ON c.id = o.id_client
GROUP BY c.name, c.last_name
ORDER BY nombre_completo ASC;


SELECT SUM(o.cant) AS cantidad_productos, p.title
...
SELECT TOP 5 SUM(o.cant) AS cantidad_productos, g.gender
FROM order_details o
JOIN products p ON o.id_product = p.id
...
SELECT CONCAT(e.name, ' ', e.last_name) AS nombre_completo, e.legajo FROM employees e
JOIN orders o ON e.id = o.id_employee
WHERE o.date between '2024-01-12' AND '2024-01-15' 
ORDER BY o.date DESC, e.legajo ASC;

--Métodos de pagos utilizados por los socios, siempre que sea mas de 2 veces

--SIN SOCIOS
SELECT pm.payment_method, COUNT(o.id_payment_method) AS metodos_de_pago
FROM payment_methods pm
JOIN orders o ON pm.id = o.id_payment_method
...
FROM orders o
WHERE o.total > (SELECT AVG(sale_price) AS precio_promedio FROM products p);
